...
function save() {
	...
	loc=window.location.href.split("/shader")[0]+"/saveshader";
	if(am_i_owner())
	...
	$.post(loc,
		...
		function(result) {
			alert(result)
			loc=window.location.href.split("/shader")[0]+"/shader#" + result
...
function load_code(hash) {
	...
	loc = window.location.href.split("/shader")[0]+'/loadshader?h='+hash
